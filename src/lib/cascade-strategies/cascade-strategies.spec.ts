...
describe("CascadeStrategies", () => {
  ...
  it("should succesfully use multiple strategies", () => {
    ...
    const test2Observable = cascadeStrategies.use(
      ["test3", "test5", "test4"],
      0,
      ""
    );
  ...
  it("should fail if all strategies fails", () => {
    const strategyMap = {
      test1: (arg1: number, arg2: string) =>
        throwError(new Error("TEST_ERROR")),
      test2: (arg1: number, arg2: string) =>
        throwError(new Error("TEST_ERROR")),
      test3: (arg1: number, arg2: string) =>
        throwError(new Error("TEST_ERROR")),
      test4: (arg1: number, arg2: string) =>
        throwError(new Error("TEST_ERROR")),
      test5: (arg1: number, arg2: string) =>
        throwError(new Error("TEST_ERROR")),
